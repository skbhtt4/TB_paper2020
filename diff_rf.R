...
# Long obs
phy_eba_mic <- readRDS("../data/Long_Obs/phy_mic_long_obs.rds")
phy_eba_rna <-  readRDS("../data/Long_Obs/phy_rna_tb.rds")
phy_eba_mic
phy_eba_rna
...
phy_gsva <-  readRDS("../GSVA/phy_GSVA_all_hallmark.rds")
...
lme_pathways <- readRDS("../Tables/LME_Pathways/lme_eba_clinical_two_week.rds")
...
imp_dt_list <- readRDS("../Figs/RF_diff_model/RF_results_pathway.rds")
...
rownames(mat_imm) <- gsub("_"," ",rownames(mat_imm))
...
pdf("../Figs/RF_diff_model/Heatmap_imp_Mic_lme_Pathways.pdf",height = 15, width = 15)
draw(ht1)
dev.off()
...
# Column Annotation
colann_dt <-as.data.frame(sel_p_ann)

# Important variable
imp_ann <- imm_var_sel[match(colann_dt$Hallmark.Name,imm_var_sel$Comp),] %>% data.frame()

comb_col_ann <- cbind(colann_dt,imp_ann)

# Full df
f_df <-  cbind(rowann_dt,mat_imm)

write.csv(f_df,"../Figs/RF_diff_model/Fig_6_data.csv")

write.csv(comb_col_ann,"../Figs/RF_diff_model/Fig_6_col_ann.csv")

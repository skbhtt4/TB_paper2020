...
phy_hallmark <- readRDS("../GSVA/phy_GSVA_all_hallmark.rds")
sample_names(phy_hallmark)



phy_hallmark <- prune_samples(sample_names(phy),phy_hallmark)
sample_data(phy_hallmark) <- sample_data(phy)


phy_pair <- phy_hallmark
...
phy_long_obs <- readRDS("../data/Long_Obs/phy_rna_tb.rds")

phy_hallmark <- readRDS("../GSVA/phy_GSVA_all_hallmark.rds")
sample_names(phy_hallmark)

phy_hallmark <- prune_samples(sample_names(phy_long_obs),phy_hallmark)
sample_data(phy_hallmark) <- sample_data(phy_long_obs)

phy_trt <-  phy_hallmark
sm_dt <- data.frame(sample_data(phy_trt))
names(sm_dt)
sm_dt <- sm_dt %>%
...
write.csv(dt_w,paste0(tab_folder,"/LME_Long_Obs_Post_Pre.csv"))
...
saveRDS(all_union,paste0(tab_folder,"/lme_eba_clinical_two_week.rds"))

...
# cont.mat <- cbind(Intercept = c(1,0,0,0,0,0) ,
#                   #Sex_M_Vs_F = c(0,0,0),
#                   HRZE_Post_Vs_Pre = c(0,1,0) ,
#                   NTZ_Post_Vs_Pre = c(0,0,1))
# 

# final_fit <-  contrasts.fit(fitDupCor, cont.mat)
# final_fit <- eBayes(final_fit)
#tmp <- decideTests(final_fit,p.value = p_cut_off)
#sum_hrze_ntz <- summary( decideTests(final_fit,p.value = p_cut_off,method = "global"))
# sum_hrze_ntz <- summary( decideTests(final_fit,p.value = p_cut_off,method = "global"))
...
hrze_vp<-ggplot() + 
  ...
  ggrepel::geom_text_repel(data= data_pvalue_less[abs(data_pvalue_less$logFC)>3,],
...
data_pvalue_less$otu_mod <- gsub('\\_'," ",data_pvalue_less$otu_mod)
ntz_vp<-ggplot() + 
  geom_point(data=mer_ntz[which(mer_ntz$global_padj>=p_cut_off),],
             aes(y=-log10(global_padj),x=logFC),
             size=2,color="black",fill='black',alpha=0.1,shape=16)+
  geom_point(data= data_pvalue_less,
             aes(y=-log10(global_padj),x=logFC,fill=Order,color=Order),
             size=2,alpha=.9,shape=16)+
  ggrepel::geom_text_repel(data= data_pvalue_less[abs(data_pvalue_less$logFC)>6,],
                           aes(y=-log10(global_padj),x=logFC,label=otu_mod))+
  scale_fill_manual(name="Order",values = mycol) +
  scale_color_manual(name="Order",values = mycol) +
  geom_hline(yintercept = -log10(p_cut_off),size = .1,linetype = "dashed")+
...
library(ggpubr)
combined<-ggarrange(hrze_vp,ntz_vp)
combined
pdf(paste(fig_folder,"volcano_plot_combined.pdf",sep = "/"),height = 5, width = 15, useDingbats = F)
print(combined)
dev.off()
...
# Row Annotation
rowann_dt <- cbind(sig_dt,match_tax_dt)
...
write.csv(f_df,paste(tab_folder,paste("Fig_3_B_data.csv",sep=""),sep = "/"))

write.csv(colann_dt,paste(tab_folder,paste("Fig_3_B_col_ann.csv",sep=""),sep = "/"))

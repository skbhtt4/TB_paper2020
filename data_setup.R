...
# Let's save it
# Create a data folder for microbiome data
...
phy_rna_all <- rna_phy
sample_data(phy_rna_all) <-  sample_data(sm_dt)

phy_rna_all <-  prune_taxa(taxa_sums(phy_rna_all)>0, phy_rna_all)

# Create a data folder for microbiome data
dir.create("../data/genes", showWarnings = TRUE,recursive = TRUE)
saveRDS(phy_rna_all,"../data/genes/phy_rna_all.rds")
...
dir.create("../data/Long_Obs",showWarnings = T)
saveRDS(phy, "../data/Long_Obs/phy_mic_long_obs.rds")
...
saveRDS(phy_fil, "../data/Long_Obs/phy_rna_tb.rds")
...
sm_dt <-  sm_dt[!sm_dt$sample == "",]
sm_dt <- sm_dt[!sm_dt$TB_status %in%c("cured","treatment"),]
sm_dt <- sm_dt[sm_dt$TB_status %in%c("community_control","family_contact"),]
rownames(sm_dt) <-  sm_dt$rna_seq_ID
...
saveRDS(rna_phy_control, "../data/Control/phy_rna_fc_cc_all.rds")

# Microbiome paired
rna_phy_paired <-  prune_samples(sample_names(phy_fc_cc),rna_phy_control)
rna_phy_paired <- prune_taxa(taxa_sums(rna_phy_paired)>0,rna_phy_paired)
saveRDS(rna_phy_paired, "../data/Control/phy_rna_fc_cc_mic_paired.rds")


# Make VST 
dds <- phyloseq_to_deseq2(rna_phy_paired, ~ sex + TB_status ) 
